...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('d_price'), 'has-success': fields.d_price && fields.d_price.valid }">
    <label for="d_price" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.product.columns.d_price') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.d_price" v-validate="'required|integer'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('d_price'), 'form-control-success': fields.d_price && fields.d_price.valid}" id="d_price" name="d_price" placeholder="{{ trans('admin.product.columns.d_price') }}">
        <div v-if="errors.has('d_price')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('d_price') }}</div>
    </div>
</div>
